...
void reshape( int w, int h )
{
    ...
    glLoadIdentity();
    // Define a area a ser ocupada pela area OpenGL dentro da Janela (Viewport/Janela de exibição)
    glViewport(0, 0, w, h);
    // Define os limites logicos da area OpenGL dentro da Janela (Window/Janela de seleção/Universo)
    glOrtho(Min.x,Max.x,
            Min.y,Max.y,
            0,1);
...
string convexPolygonInclusion(int* counter)
{
    ...
    int i = 0; // polygon index (TODO: implement logic to find this out)
        ...
        {
            Poligono poligono = Voro.getPoligono(i); // ObtŽm o pol’gono atual
            Envelope enve = Voro.getEnvelope(i); //ObtŽm o ennvelope desse poligno
